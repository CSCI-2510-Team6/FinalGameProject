...
public class Sorceress extends CollidableSprite {
	...
	/**
	 * Check if character is grounded and in stand state
	 * 
	 * @return boolean
	 */
	public boolean canAttack() {
		return !inAir && (action == Action.STAND);
	}
	...
	public void updateWorld(final float delta) {
		...
		if ((action == Action.JUMP)) {
			final Vector2f shiftedCenter = getCenterPosition()
					.add(new Vector2f(0, 0f));
	...
	private void initAnimations() {
		...
		standRight[0] = Utility.scaleImage(
		...
		jumpRight[1] = Utility.scaleImage(
				...
				desiredHeight);
		meleeLeft[0] = Utility.scaleImage(
				getImage().getSubimage(0, height * 7, width, height), desiredWidth,
				desiredHeight);
		...
		meleeLeft[4] = Utility.scaleImage(
				...
				desiredWidth, desiredHeight);
		meleeRight[0] = Utility.scaleImage(
				getImage().getSubimage(0, height * 5, width, height), desiredWidth,
				desiredHeight);
